...
class TourContext():
    ...
    def __init__(self, tour_name: str) -> None:
        self.__tour = Tour.objects.filter(tour_name=tour_name)

    def get_tour(self) -> list:
        return [tour for tour in self.__tour]

    def get_tour_name(self) -> list:
        return self.__tour[0].tour_name

    def get_tour_price(self) -> list:
        return self.__tour[0].tour_price

    def get_tour_duration(self) -> list:
        return self.__tour[0].tour_duration

    def get_tour_description(self) -> list:
        return self.__tour[0].tour_description

    def get_tour_max_participants(self) -> list:
        return self.__tour[0].max_participants

    def get_tour_rating(self) -> list:
        return [Rating.objects.filter(pk=tour.rating.pk)[0] for tour in TourRating.objects.filter(tour__in=self.__tour)[:1]]

    def get_tour_images(self) -> list:
        return [tour_images for tour_images in TourImage.objects.filter(tour__in=self.__tour)[:4]]

    def get_tour_instructor(self) -> list:
        return [Instructor.objects.filter(pk=tour_instructor.instructor.pk)[0] for tour_instructor in TourInstructor.objects.filter(tour__in=self.__tour)]
